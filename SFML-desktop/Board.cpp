...
Board::Board()
	...
	left(sf::Vector2f(dim.x / 2, dim.y * size.y)),
	right(sf::Vector2f(dim.x / 2, dim.y * size.y))
{
	top.setPosition(offset.x, offset.y - dim.y + dim.y / 2);
	bottom.setPosition(offset.x, offset.y + size.y * dim.y);
	left.setPosition(offset.x - dim.x + dim.x / 2, offset.y);
	right.setPosition(offset.x + size.x * dim.x, offset.y);
}
